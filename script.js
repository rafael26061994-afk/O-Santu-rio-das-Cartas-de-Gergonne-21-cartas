// BANCO DE DADOS COMPLETO COM AS 52 CARTAS DO BARALHO (SEM CORINGAS)
const FULL_52_DECK = [
    // Espadas ♠
...
function showTableScreen() {
    ...
    // EMBARALHAMENTO DO MAÇO DE 52 E RETIRADA DE EXATAMENTE 21 CARTAS ALEATÓRIAS
...
function revealMind() {
    ...
    const finalCard = createCardHTML(winningCard);
    finalWrapper.appendChild(finalCard);
...
function readText() {
    ...
    if (activeScreen.id === "screen-table") {
        textToRead = "Mesa disposta com três colunas verticais de alta definição contendo vinte e uma cartas sorteadas. Localize a sua e clique no botão da coluna correspondente.";
    } else {
